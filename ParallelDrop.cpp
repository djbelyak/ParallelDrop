...
void display() //вот и та функция дисплея
{
    ...
    glColor3f(1.0, 0.0, 0.0);// устанавливаем цвет которым будем рисовать (если вы рисуете много точек или линий то можно пере тем как нарисовать что- либо установить для него цвет, если вам не устраивает цвет то 0 до 1 то можно установить glColor3ub и тогда будут 255 цвета
...
int main(int argc, char **argv)//точка входа приложения
{
    ...
    glutDisplayFunc(display);//указываем функцию дисплея(ну или рисования)
